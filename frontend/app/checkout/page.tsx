...
export default function CheckoutPage() {
    ...
    const handlePayment = async (e: React.FormEvent) => {
        ...
        if (!city || city === "Select City") return toast.error("Please select a city for your event");
        setLoading(true);
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            ...
                            <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-zinc-100">
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    ...
                                    <div className="space-y-1">
                                        <Label className="text-[11px] font-bold uppercase text-zinc-500">City / Location</Label>
                                        <select
                                            className={`flex h-12 w-full rounded-xl border-0 px-3 py-2 text-sm ring-offset-background focus:outline-none focus:ring-2 focus:ring-black transition-all ${city === "Select City" ? "bg-[#D4AF37]/10 ring-2 ring-[#D4AF37]" : "bg-zinc-50"
                                                }`}
                                            value={city}
                                            onChange={(e) => setCity(e.target.value)}
                                            required
                                        >
                                            <option value="Select City">Choose City</option>
                                            {CITIES.map(c => (
