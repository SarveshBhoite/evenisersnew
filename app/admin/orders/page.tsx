                ...
                >
                  ...
                  <td className="px-8 py-5">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent
                        align="start"
                        className="bg-white rounded-xl shadow-xl border-zinc-100 p-2 z-50"
                      >
                        ...
                        <DropdownMenuItem
                          ...
                          className="cursor-pointer font-medium text-green-700 focus:bg-green-50 rounded-lg"
                        >
                          <Check className="w-4 h-4 mr-2" /> Mark Completed
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => changeStatus(order._id, "cancelled")}
                          className="cursor-pointer font-medium text-red-700 focus:bg-red-50 rounded-lg"
                        >
                          <XCircle className="w-4 h-4 mr-2" /> Mark Cancelled
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem
                          onClick={() => changeStatus(order._id, "in_progress")}
                          className="cursor-pointer font-medium text-blue-700 focus:bg-blue-50 rounded-lg"
                        >
...
function getStatusColor(status: string) {
  switch (status) {
    case "paid":
      return "bg-green-100 text-green-700 border-green-200 uppercase";
    case "partial_paid":
      return "bg-orange-100 text-orange-700 border-orange-200 uppercase";
    case "broadcasting":
      return "bg-purple-100 text-purple-700 border-purple-200 animate-pulse uppercase";
    case "in_progress":
      return "bg-blue-100 text-blue-700 border-blue-200 uppercase";
    case "completed":
      return "bg-zinc-800 text-white border-zinc-800 uppercase";
    case "cancelled":
      return "bg-red-100 text-red-700 border-red-200 uppercase";
    default:
      return "bg-gray-100 text-gray-700 border-gray-200 uppercase";
  }
}
...
function OrderDetailsModal({
  ...
  const fetchLocalVendors = async () => {
    ...
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/vendors?city=${order.shippingAddress.city}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleBroadcast = async () => {
    ...
    try {
      await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/api/orders/${order._id}/broadcast`,
  ...
  const sendWhatsApp = (vendor: any) => {
    ...
    window.open(
      `https://wa.me/${vendor.phone}?text=${encodeURIComponent(msg)}`,
      "_blank"
    );
  };
    ...
    <Dialog>
      <DialogTrigger asChild>
        <Button
          variant="ghost"
          size="sm"
          className="gap-2 rounded-full hover:bg-zinc-100"
          onClick={fetchLocalVendors}
        >
        ...
        <DialogHeader className="p-8 pb-6 border-b border-zinc-100 bg-zinc-50/50 flex flex-row justify-between items-start">
          <div className="space-y-1">
            <DialogTitle className="font-serif text-3xl font-bold flex items-center gap-3">
              ...
              <Badge
                variant="outline"
                className="font-sans font-normal text-sm px-3 py-1 border-zinc-300"
              >
                #{order._id.slice(-8).toUpperCase()}
              </Badge>
            </DialogTitle>
            <p className="text-zinc-400 text-xs uppercase tracking-widest font-bold">
              Review details & assign responsibilities
            </p>
          </div>
              ...
              <div className="flex flex-col items-end gap-2 relative">
                ...
                {assignMode === "myself" && (
                  <Button
                    ...
                    disabled={loading}
                    size="sm"
                    className="w-[240px] bg-black text-white rounded-xl h-9 animate-in slide-in-from-top-2 fade-in"
                  >
                    ...
                    <div className="max-h-40 overflow-y-auto space-y-1 mb-2 custom-scrollbar">
                      {vendors.length > 0 ? (
                        vendors.map((v: any) => (
                          <div
                            key={v._id}
                            className="flex items-center justify-between p-2 hover:bg-zinc-50 rounded-lg group"
                          >
                            <div
                              className="flex items-center gap-2 cursor-pointer max-w-[80%]"
                            ...
                            <div
                              ...
                              title="WhatsApp"
                              className="p-1.5 rounded-full bg-green-50 text-green-600 hover:bg-green-100 cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity"
                            >
          ...
          {order.assignedVendor && (
            <Badge className="bg-green-100 text-green-800 border-green-200 px-4 py-2 text-xs font-bold uppercase tracking-wider">
              <Check className="w-3 h-3 mr-2" /> Assigned:{" "}
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              ...
              <div className="text-sm bg-zinc-50 p-6 rounded-2xl border border-zinc-100 space-y-3">
                ...
                <div className="pt-2">
                  <Badge
                    className={`w-full justify-center py-1 ${getStatusColor(
                      order.status
                    )}`}
                  >
              ...
              {order.items.map((item: any, idx: number) => (
                ...
                >
                  ...
                  <div className="bg-white border border-zinc-100 rounded-xl p-4 flex flex-wrap gap-4 text-sm mt-2 shadow-sm">
                    ...
                    {item.message && (
                      <div className="w-full text-zinc-600 border-t border-zinc-100 pt-2 mt-1 italic">
                        "{item.message}"
                      </div>
                    )}
                  </div>
